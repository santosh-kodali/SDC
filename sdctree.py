...
class tree:
    ...
    def show_robdd(self,node,dot):
    	if(node != None):
    		print(node.getId())
    		dot.node(str(node.getId()),node.getValue()+str(node.getData())+str(node.getlogic())+'\t'+ str(node.getId()))
	    	if(node.getlChild() != None):
	    		dot.edge(str(node.getId()),str(node.getlChild().getId()))
	    		dot.edge(str(node.getId()),str(node.getrChild().getId()))
